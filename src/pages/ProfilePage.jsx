...
import { getUserProfile, getUserPurchases, getUserGifts } from "../api/authApi";
    ...
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-10">
      ...
      <Card className="bg-white shadow border border-pink-200">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-2xl font-semibold text-blue-900">Purchased Videos</h2>
          </div>
          {purchases.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {purchases.map((item) => (
                ...
                >
                  <h3 className="text-lg font-medium text-blue-900">{item._id.title}</h3>
            ...
            <ul className="space-y-2">
              {gifts.map((gift) => (
                <li key={gift._id} className="border-b pb-2 text-gray-700">
                  ₹{gift.amount} to <span className="font-semibold text-pink-700">{gift.creatorName}</span> — for <span className="italic">{gift.videoTitle}</span>
                </li>
              ))}
            </ul>
          )}
